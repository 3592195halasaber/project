...
export default function Brands() {
  ...
  return (
    <>
      <h2 className="text-5xl mt-10 text-blue-400 text-center">All Brands</h2>
        ...
        {brands.length > 0 ? (
          brands.map((brand) => (
            ...
            >
              ...
              <h3 className="text-lg text-blue-500 font-bold text-center mt-2">{brand.name}</h3>
            </div>
          ))
        ) : (
      ...
      <Modal show={openModal} onClose={() => setOpenModal(false)}>
        <Modal.Header className="bg-blue-400">
        ...
        <Modal.Body>
          {selectedBrand ? (
            <div className="flex flex-col items-center">
              ...
              />
              <h2 className="text-2xl text-blue-600 font-bold">{selectedBrand.name}</h2>
            </div>
          ) : (
