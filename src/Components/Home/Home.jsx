...
export default function Home() {
  ...
  function RendumProudect() {
    ...
    var Item = data?.data?.data?.filter((product) => {
      return serch.toLowerCase() === '' ? product : product.title?.toLowerCase().includes(serch);
    }).map((product) => (
      <ProudectCart
        key={product._id}
        product={product}
        onAddToCart={() => AddProudect(product._id)}
        onAddToWishlist={() => handleClick(product._id)}
      />
    ));
    return Item;
  ...
  return <>
    ...
    {isLoading && <p className="text-center text-lg text-blue-500">Loading...</p>}
